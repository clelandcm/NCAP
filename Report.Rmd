...
ggplot(enroll_new_m[day <= Sys.Date() & type %in% c('Cumulative Enrolled','Cumulative Target'),], 
  ...
  geom_path() +
  scale_y_continuous(limits=c(0,figtop), breaks=seq(0,figtop,12)) +
...
PC[coupon == "AUGL02098_003", coupon := "AUGL020298-003"]
...
pcomp <- merge(pcomp, PC[Eligible == TRUE, .(ncoupons = .N), by = .(participant_id)], all.x = TRUE, all.y = FALSE)
...
label(pcomp$ncoupons) <- "Number of Peers Enrolled"
label(pcomp$coupons_cat) <- "Number of Peers Enrolled (Categorical)"
label(pcomp$Any_Peer) <- "Any Peers Enrolled"
```

## Enrollment of Peers among Participants Assigned to Peer Component
```{r}
