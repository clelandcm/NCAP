...
```{r, eval=FALSE}
DT_rand <- DT[redcap_event_name == "randomization_arm_2",.(participant_id, rand_date, rand_condition, randomization_complete)]

DT_bl <- DT[redcap_event_name == "baseline_interview_arm_2",.(participant_id, bl_date = as.Date(format(n_baseline_end, "%Y-%m-%d")), baseline_complete)]

DT_enroll <- merge(DT_rand, DT_bl, all = TRUE)

DT_enroll[is.na(rand_date),.(participant_id,bl_date,baseline_complete,rand_date, rand_condition, randomization_complete)]

with(DT_enroll, table(bl_complete = baseline_complete == 2, 
                      rnd_complete = randomization_complete == 2, 
                      rnd_date = !is.na(rand_date), 
                      useNA = 'always')) |> 
  as.data.frame() |> 
  subset(Freq > 0)
```
...
enroll_new <- data.frame(day = seq(as.Date("2022-07-01"), as.Date("2023-12-01"), 1))

setDT(enroll_new)

DT[, bl_date := as.Date(format(n_baseline_end, "%Y-%m-%d"))]
DT[, rand_date := as.Date(format(rand_date, "%Y-%m-%d"))]

enroll_new <- merge(enroll_new, DT[!is.na(rand_date), .(Enrolled = .N), by = .(rand_date)], by.x = "day", by.y = "rand_date", all=TRUE)
...
-   The NCAP enrollment target (n=448) was reached on November 15, 2023 
...
```{r}
Enrolled <- subset(DT, (redcap_event_name == "randomization_arm_2"))$participant_id
```
...
table1(~ sex_Birth + edu_years_of_school +    
       ...
       data = DT[redcap_event_name == "baseline_interview_arm_2" & participant_id %in% Enrolled,])
```
...
table1(~ ., data = DT[grepl("baseline_interview_arm_2", redcap_event_name) & participant_id %in% Enrolled, 
...
table1(~ cov_pan_chal_hlth_2 + covid_pandemic_challenges_abod_2 + covid_pandemic_challenges_food_2 + 
       ...
       data = DT[grepl("baseline_interview_arm_2", redcap_event_name) & participant_id %in% Enrolled])
```
...
table1(~ Trust_Total, data = trust_n[participant_id %in% Enrolled])
```
...
table1(~ Counter_Narratives_Total, data = counter_n[participant_id %in% Enrolled])
```
...
table1(~ Social_Norms_Total, data = snorms_n[participant_id %in% Enrolled])
```
...
DT[, Pounds := as.numeric(Pounds)]
DT[Pounds < 50 | Pounds > 400, Pounds := NA]
...
## Completion
```{r}
DT[redcap_event_name == "baseline_interview_arm_2" & participant_id %in% Enrolled, Days_Since_BL := as.numeric(difftime(Sys.Date(), bl_date, units = "days"))]

FUrate <- DT[redcap_event_name == "baseline_interview_arm_2" & participant_id %in% Enrolled, .(participant_id, bl_date, Days_Since_BL)]
...
FUrate[, FU1_Completion := factor(ifelse(FU1 == "Yes" | !is.na(fu1_date), "Completed", 
                                         ifelse(Days_Since_BL > 57, "Follow-Up One Missed", NA)))]

FUrate[, FU2_Completion := factor(ifelse(FU2 == "Yes" | !is.na(fu2_date), "Completed",
...
```{r}
label(DT$redcap_event_name) <- "REDCap Event Name"              
label(DT$unique_id_complete) <- "Unique Identifier Complete"             
label(DT$screening_consent_form_complete) <- "Screening Consent Form Complete"
label(DT$did_participant_consent) <- "Did participant consent"        
label(DT$re_screen) <- "" 
label(DT$race_ethn_race___1) <- "American Indian or Alaska Native"             
label(DT$race_ethn_race___2) <- "Black or African American"             
label(DT$race_ethn_race___3) <- "Asian"             
label(DT$race_ethn_race___4) <- "Native Hawaiian or  Other Pacific Islander"             
label(DT$race_ethn_race___5) <- "White"             
label(DT$race_ethn_race___15) <- "Some other race"            
label(DT$race_ethn_race___99) <- "Prefer not to answer"
label(DT$race_ethn_asian_detail_2___1) <- "Japanese"   
label(DT$race_ethn_asian_detail_2___2) <- "Filipino"   
label(DT$race_ethn_asian_detail_2___3) <- "Chinese"   
label(DT$race_ethn_asian_detail_2___4) <- "Korean"   
label(DT$race_ethn_asian_detail_2___5) <- "Other Asian"   
label(DT$race_ethn_asian_detail_2___99) <- "Prefer not to answer"  
label(DT$race_ethn_islander_detail_2___1) <- "Native Hawaiian"
label(DT$race_ethn_islander_detail_2___2) <- "Pacific Islander"
label(DT$race_ethn_islander_detail_2___3) <- "Samoan"
label(DT$race_ethn_islander_detail_2___4) <- "Tongan"
label(DT$race_ethn_islander_detail_2___5) <- "Maori"
label(DT$race_ethn_islander_detail_2___6) <- "Fijian"
label(DT$race_ethn_islander_detail_2___7) <- "Chamorro"
label(DT$race_ethn_islander_detail_2___8) <- "Chuukese"
label(DT$race_ethn_islander_detail_2___9) <- "Kosraen"
label(DT$race_ethn_islander_detail_2___10) <- "Marshallese"
label(DT$race_ethn_islander_detail_2___11) <- "Palauan"
label(DT$race_ethn_islander_detail_2___12) <- "Pohnpeian"
label(DT$race_ethn_islander_detail_2___13) <- "Yapese"
label(DT$race_ethn_islander_detail_2___14) <- "Other Pacific Islander"
label(DT$race_ethn_islander_detail_2___99) <- "Prefer not to answer"
label(DT$race_ethn_hispanic_detail_2___1) <- "Mexican, Mexican Am., Chicano"
label(DT$race_ethn_hispanic_detail_2___2) <- "Puerto Rican"
label(DT$race_ethn_hispanic_detail_2___3) <- "Cuban"
label(DT$race_ethn_hispanic_detail_2___4) <- "Another Hispanic, Latino, or Spanish origin"
label(DT$race_ethn_hispanic_detail_2___5) <- "Salvadoran"
label(DT$race_ethn_hispanic_detail_2___6) <- "Dominican"
label(DT$race_ethn_hispanic_detail_2___7) <- "Colombian"
label(DT$race_ethn_hispanic_detail_2___99) <- "Prefer not to answer"
label(DT$tested_past_3_mo) <- ""               
label(DT$vax_past_year) <- ""                  
label(DT$consent_form_complete) <- "Consent form complete"
label(DT$n_loc_q3___1) <- "Spouse/partner"                   
label(DT$n_loc_q3___2) <- "Boyfriend/girlfriend"                   
label(DT$n_loc_q3___3) <- "Friend"                   
label(DT$n_loc_q3___4) <- "Parent"                   
label(DT$n_loc_q3___5) <- "Child"                   
label(DT$n_loc_q3___6) <- "Other family member"                   
label(DT$n_loc_q3___98) <- "Other"                  
label(DT$n_loc_q10___1) <- "Parent"                  
label(DT$n_loc_q10___2) <- "Child"                  
label(DT$n_loc_q10___3) <- "Other family member"                  
label(DT$n_loc_q10___98) <- "Other"
label(DT$n_loc_q17___1) <- "Spouse/partner"                  
label(DT$n_loc_q17___2) <- "Boyfriend/girlfriend"                  
label(DT$n_loc_q17___3) <- "Friend"                  
label(DT$n_loc_q17___4) <- "Parent"                  
label(DT$n_loc_q17___5) <- "Child"                  
label(DT$n_loc_q17___6) <- "Other family member"                  
label(DT$n_loc_q17___98) <- "Other"
label(DT$locator_form_complete) <- "Locator form complete"          
label(DT$parent) <- ""                         
label(DT$symptoms_complete) <- ""              
label(DT$sym_date_mdy) <- ""                   
label(DT$n_practice7___1) <- "Fruits"                
label(DT$n_practice7___2) <- "Vegetables"                
label(DT$n_practice7___3) <- "Sweets"
label(DT$baseline_complete) <- "Baseline interview complete"              
label(DT$rand_date) <- "Date of random assignment to study condition"                      
label(DT$randomization_complete) <- "Randomization form complete"         
label(DT$core_session_complete) <- "Core session complete"          
label(DT$component_a_mi_session_complete) <- ""
label(DT$component_b_tmqq_complete) <- ""      
label(DT$peer_race_ethn_race___1) <- "American Indian"        
label(DT$peer_race_ethn_race___2) <- "Black or African American"        
label(DT$peer_race_ethn_race___3) <- "Asian"        
label(DT$peer_race_ethn_race___4) <- "Native Hawaiian or Other Pacific Islander"        
label(DT$peer_race_ethn_race___5) <- "White"        
label(DT$peer_race_ethn_race___15) <- "Some other race"
label(DT$peer_occupation___1) <- "Food preparation and serving (e.g., deli, bodega, restaurants, fast food)"            
label(DT$peer_occupation___2) <- "Retail and sales (e.g., grocery, drug, and convenience stores)"            
label(DT$peer_occupation___3) <- "Building and grounds cleaning and maintenance"            
label(DT$peer_occupation___4) <- "Personal care and service (e.g., in-home childcare workers, barbers, nail technicians, cosmetologists)"            
label(DT$peer_occupation___5) <- "In-home health care services (e.g., home health aides)"            
label(DT$fu1_q7___a) <- "A) BD Veritor At-Home COVID-19 Test"                     
label(DT$fu1_q7___b) <- "B) BinaxNOW COVID-19 Antigen Self Test"                     
label(DT$fu1_q7___c) <- "C) CareStart COVID-19 Antigen Home Test"                     
label(DT$fu1_q7___d) <- "D) Flowflex COVID-19 Antigen Home Test"                     
label(DT$fu1_q7___e) <- "E) iHealth COVID-19 Antigen Rapid Test"                     
label(DT$fu1_q7___f) <- "F) InteliSwab COVID-19 Rapid Test"                     
label(DT$fu1_q7___g) <- "G) Quidel Quickvue At-Home OTC COVID-19 test"                     
label(DT$fu1_q7___h) <- "H) NONE OF THESE (continue)"
label(DT$fu1_q8___i) <- "I) Celltrion DiaTrust COVID-19 Ag Home Test"                     
label(DT$fu1_q8___j) <- "J) CLINITEST Rapid COVID-19 Antigen Self-Test"                     
label(DT$fu1_q8___k) <- "K) Cue COVID-19 Test for Home and Over The Counter (OTC) Use"                     
label(DT$fu1_q8___l) <- "L) Detect Covid-19 Test"                     
label(DT$fu1_q8___m) <- "M) Ellume COVID-19 Home Test"                     
label(DT$fu1_q8___n) <- "N) Lucira CHECK-IT COVID-19 Test Kit"                     
label(DT$fu1_q8___o) <- "None of these"                     
label(DT$fu1_q8___p) <- "Some other rapid test not on the handout"                     
label(DT$fu1_q8___q) <- "Does not remember which rapid test used"
label(DT$serial_rapid_test) <- ""              
label(DT$fu1_q22___1) <- "Forgot"                    
label(DT$fu1_q22___2) <- "Figured they would call me"                    
label(DT$fu1_q22___3) <- "I tried but was not able to"                    
label(DT$fu1_q22___4) <- "Did not want to know"                    
label(DT$fu1_q22___5) <- "It would be inconvenient to find out I am positive"                    
label(DT$fu1_q22___6) <- "Other reason (specify)"
label(DT$fu1_q23___1) <- "A doctor's office or health clinic"                    
label(DT$fu1_q23___2) <- "Community-based organization"                    
label(DT$fu1_q23___3) <- "A laboratory"                    
label(DT$fu1_q23___4) <- "A standalone testing site"                    
label(DT$fu1_q23___5) <- "Pop-up clinic on the street or sidewalk"                    
label(DT$fu1_q23___6) <- "Street fair"                    
label(DT$fu1_q23___7) <- "Mobile van"                    
label(DT$fu1_q23___8) <- "Hospital"                    
label(DT$fu1_q23___9) <- "Other (specify)"
label(DT$fu1_q24___1) <- "Called the Test to Treat phone number"                    
label(DT$fu1_q24___2) <- "Contacted a health care provider"                    
label(DT$fu1_q24___3) <- "Had a visit with a health care provider"                    
label(DT$fu1_q24___4) <- "Took medications such as Paxlovid"                    
label(DT$fu1_q24___5) <- "Had to miss work"                    
label(DT$fu1_q24___6) <- "Had to skip social events"                    
label(DT$fu1_q24___7) <- "Was hospitalized"                    
label(DT$fu1_q24___8) <- "Other effects? (Specify)"
label(DT$fu1_q16___1) <- "It was required by work"                    
label(DT$fu1_q16___2) <- "It was required by another organization (e.g., airline)"                    
label(DT$fu1_q16___3) <- "A friend or family member asked me to"                    
label(DT$fu1_q16___4) <- "I just wanted to know"                    
label(DT$fu1_q16___5) <- "I test myself regularly even if I do not have symptoms"                    
label(DT$fu1_q16___6) <- "I was preparing for upcoming event where others would be present"                    
label(DT$fu1_q16___7) <- "I had symptoms of COVID"                    
label(DT$fu1_q16___8) <- "I was exposed to someone with COVID"                    
label(DT$fu1_q16___9) <- "I had a trip coming up"                    
label(DT$fu1_q16___10) <- "I returned home from a trip"                   
label(DT$fu1_q16___11) <- "My rapid self-test was positive"                   
label(DT$fu1_q16___12) <- "I had a diagnosis of COVID"                   
label(DT$fu1_q16___13) <- "I had COVID and I needed to know if I could end a quarantine period"                   
label(DT$fu1_q16___14) <- "Another reason (specify)"
label(DT$fu1_q94___1) <- "I did not have a good reason to test"                    
label(DT$fu1_q94___2) <- "I did not want to get tested"                    
label(DT$fu1_q94___3) <- "I did not have access to a rapid self-test kit"                    
label(DT$fu1_q94___4) <- "I could not afford to get tested"                    
label(DT$fu1_q94___5) <- "I don't trust the accuracy of the tests"                    
label(DT$fu1_q94___6) <- "I don't want my test results reported to the health department"                    
label(DT$fu1_q94___7) <- "I could not afford to miss work if I tested positive"                    
label(DT$fu1_q94___8) <- "I could not isolate myself if I tested positive"                    
label(DT$fu1_q94___9) <- "I did not have any COVID symptoms"                    
label(DT$fu1_q94___10) <- "Another reason (Specify)"
label(DT$fu1_q59___1) <- "I don't have to worry about getting COVID-19"                    
label(DT$fu1_q59___2) <- "I don't have COVID-19 now"                    
label(DT$fu1_q59___3) <- "I can be around others without giving the virus to them"                    
label(DT$fu1_q59___4) <- "I can be around others without getting the virus from them"                    
label(DT$fu1_q59___99) <- "Prefer not to answer"
label(DT$fu1_q60___1) <- "I will need to be admitted to the hospital"                    
label(DT$fu1_q60___2) <- "I will need to isolate myself from others"                    
label(DT$fu1_q60___3) <- "I will need to take off work"                    
label(DT$fu1_q60___99) <- "Prefer not to answer"                   
label(DT$fu1_q69___1) <- "Need to take time off work to get tested"                    
label(DT$fu1_q69___2) <- "Out of pocket costs for test"                    
label(DT$fu1_q69___3) <- "Out of pocket costs for transportation, childcare, or time off work to get tested"                    
label(DT$fu1_q69___4) <- "I do not know where to go to be tested"                    
label(DT$fu1_q69___5) <- "Pain or discomfort from the test"                    
label(DT$fu1_q69___6) <- "Saliva"                    
label(DT$fu1_q69___7) <- "Concern about others handling my personal data"                    
label(DT$fu1_q69___8) <- "Other"                    
label(DT$fu1_q69___9) <- "None of the above"                    
label(DT$fu1_q69___98) <- "Don't know"                   
label(DT$fu1_q69___99) <- "Prefer not to answer"
label(DT$fu1_q71___1) <- "There are other people in my household"                    
label(DT$fu1_q71___2) <- "There are children in my household"                    
label(DT$fu1_q71___3) <- "There are older adults in my household"                    
label(DT$fu1_q71___4) <- "I don't have a good place where I could isolate"                    
label(DT$fu1_q71___5) <- "No one to help me if I am sick"                    
label(DT$fu1_q71___6) <- "Lost income or wages"                    
label(DT$fu1_q71___7) <- "People might blame me or treat me badly"                    
label(DT$fu1_q71___8) <- "Other"                    
label(DT$fu1_q71___9) <- "None of the above"                    
label(DT$fu1_q71___98) <- "Don't know"                   
label(DT$fu1_q71___99) <- "Prefer not to answer"                   
label(DT$fu1_q40___1) <- "I want to keep my family safe"                    
label(DT$fu1_q40___2) <- "I want to keep my community safe I want to keep myself safe"                    
label(DT$fu1_q40___3) <- "I have a chronic health problem, like asthma or diabetes"                    
label(DT$fu1_q40___4) <- "My doctor told me to get a COVID-19 vaccine"                    
label(DT$fu1_q40___5) <- "I don't want to get really sick from COVID-19"                    
label(DT$fu1_q40___6) <- "I want to feel safe around other people"                    
label(DT$fu1_q40___7) <- "I believe life won't go back to normal until most people get a COVID-19 vaccine"        
label(DT$fu1_q40___8) <- "Required by my school or workplace"                    
label(DT$fu1_q40___9) <- "Required for travel"                    
label(DT$fu1_q40___10) <- "Other"                   
label(DT$fu1_q40___11) <- "Not Applicable"                   
label(DT$fu1_q40___12) <- "Prefer not to answer"
label(DT$fu1_q41___1) <- "I'm allergic to vaccines"                    
label(DT$fu1_q41___2) <- "I don't like needles"                    
label(DT$fu1_q41___3) <- "I'm not concerned about getting really sick from COVID-19"                    
label(DT$fu1_q41___4) <- "I'm concerned about side effects from the vaccine"                    
label(DT$fu1_q41___5) <- "I don't think vaccines work very well"                    
label(DT$fu1_q41___6) <- "I don't trust that the vaccine will be safe"                    
label(DT$fu1_q41___7) <- "I don't believe the COVID- 19 pandemic is as bad as some people say it is"                    
label(DT$fu1_q41___8) <- "I don't want to pay for it"                    
label(DT$fu1_q41___9) <- "I don't know enough about how well a COVID-19 vaccine works"                    
label(DT$fu1_q41___10) <- "Other"                   
label(DT$fu1_q41___11) <- "Not Applicable"                   
label(DT$fu1_q41___12) <- "Prefer not to answer"
label(DT$mi_q1) <- ""                          
label(DT$mi_q2) <- ""                          
label(DT$mi_q3) <- ""                          
label(DT$tmqq_q1) <- ""                        
...
label(DT$peer_q3) <- ""                        
label(DT$peer_q4) <- ""                        
label(DT$self_test_q1) <- ""                   
label(DT$self_test_q2) <- ""                   
label(DT$nav_q1) <- ""                         
label(DT$nav_q2) <- ""                         
label(DT$fu_soc_harms_q_1) <- ""               
label(DT$fu_soc_harms_explained) <- ""         
label(DT$fu1_q7_fu2___a) <- "A) BD Veritor At-Home COVID-19 Test"                     
label(DT$fu1_q7_fu2___b) <- "B) BinaxNOW COVID-19 Antigen Self Test"                     
label(DT$fu1_q7_fu2___c) <- "C) CareStart COVID-19 Antigen Home Test"                     
label(DT$fu1_q7_fu2___d) <- "D) Flowflex COVID-19 Antigen Home Test"                     
label(DT$fu1_q7_fu2___e) <- "E) iHealth COVID-19 Antigen Rapid Test"                     
label(DT$fu1_q7_fu2___f) <- "F) InteliSwab COVID-19 Rapid Test"                     
label(DT$fu1_q7_fu2___g) <- "G) Quidel Quickvue At-Home OTC COVID-19 test"                     
label(DT$fu1_q7_fu2___h) <- "H) NONE OF THESE (continue)"
label(DT$fu1_q8_fu2___i) <- "I) Celltrion DiaTrust COVID-19 Ag Home Test"                     
label(DT$fu1_q8_fu2___j) <- "J) CLINITEST Rapid COVID-19 Antigen Self-Test"                     
label(DT$fu1_q8_fu2___k) <- "K) Cue COVID-19 Test for Home and Over The Counter (OTC) Use"                     
label(DT$fu1_q8_fu2___l) <- "L) Detect Covid-19 Test"                     
label(DT$fu1_q8_fu2___m) <- "M) Ellume COVID-19 Home Test"                     
label(DT$fu1_q8_fu2___n) <- "N) Lucira CHECK-IT COVID-19 Test Kit"                     
label(DT$fu1_q8_fu2___o) <- "None of these"                     
label(DT$fu1_q8_fu2___p) <- "Some other rapid test not on the handout"                     
label(DT$fu1_q8_fu2___q) <- "Does not remember which rapid test used"
label(DT$serial_rapid_test) <- ""              
label(DT$fu1_q22_fu2___1) <- "Forgot"                    
label(DT$fu1_q22_fu2___2) <- "Figured they would call me"                    
label(DT$fu1_q22_fu2___3) <- "I tried but was not able to"                    
label(DT$fu1_q22_fu2___4) <- "Did not want to know"                    
label(DT$fu1_q22_fu2___5) <- "It would be inconvenient to find out I am positive"                    
label(DT$fu1_q22_fu2___6) <- "Other reason (specify)"
label(DT$fu1_q23_fu2___1) <- "A doctor's office or health clinic"                    
label(DT$fu1_q23_fu2___2) <- "Community-based organization"                    
label(DT$fu1_q23_fu2___3) <- "A laboratory"                    
label(DT$fu1_q23_fu2___4) <- "A standalone testing site"                    
label(DT$fu1_q23_fu2___5) <- "Pop-up clinic on the street or sidewalk"                    
label(DT$fu1_q23_fu2___6) <- "Street fair"                    
label(DT$fu1_q23_fu2___7) <- "Mobile van"                    
label(DT$fu1_q23_fu2___8) <- "Hospital"                    
label(DT$fu1_q23_fu2___9) <- "Other (specify)"
label(DT$fu1_q24_fu2___1) <- "Called the Test to Treat phone number"                    
label(DT$fu1_q24_fu2___2) <- "Contacted a health care provider"                    
label(DT$fu1_q24_fu2___3) <- "Had a visit with a health care provider"                    
label(DT$fu1_q24_fu2___4) <- "Took medications such as Paxlovid"                    
label(DT$fu1_q24_fu2___5) <- "Had to miss work"                    
label(DT$fu1_q24_fu2___6) <- "Had to skip social events"                    
label(DT$fu1_q24_fu2___7) <- "Was hospitalized"                    
label(DT$fu1_q24_fu2___8) <- "Other effects? (Specify)"
label(DT$fu1_q16_fu2___1) <- "It was required by work"                    
label(DT$fu1_q16_fu2___2) <- "It was required by another organization (e.g., airline)"                    
label(DT$fu1_q16_fu2___3) <- "A friend or family member asked me to"                    
label(DT$fu1_q16_fu2___4) <- "I just wanted to know"                    
label(DT$fu1_q16_fu2___5) <- "I test myself regularly even if I do not have symptoms"                    
label(DT$fu1_q16_fu2___6) <- "I was preparing for upcoming event where others would be present"                    
label(DT$fu1_q16_fu2___7) <- "I had symptoms of COVID"                    
label(DT$fu1_q16_fu2___8) <- "I was exposed to someone with COVID"                    
label(DT$fu1_q16_fu2___9) <- "I had a trip coming up"                    
label(DT$fu1_q16_fu2___10) <- "I returned home from a trip"                   
label(DT$fu1_q16_fu2___11) <- "My rapid self-test was positive"                   
label(DT$fu1_q16_fu2___12) <- "I had a diagnosis of COVID"                   
label(DT$fu1_q16_fu2___13) <- "I had COVID and I needed to know if I could end a quarantine period"                   
label(DT$fu1_q16_fu2___14) <- "Another reason (specify)"
label(DT$fu1_q94_fu2___1) <- "I did not have a good reason to test"                    
label(DT$fu1_q94_fu2___2) <- "I did not want to get tested"                    
label(DT$fu1_q94_fu2___3) <- "I did not have access to a rapid self-test kit"                    
label(DT$fu1_q94_fu2___4) <- "I could not afford to get tested"                    
label(DT$fu1_q94_fu2___5) <- "I don't trust the accuracy of the tests"                    
label(DT$fu1_q94_fu2___6) <- "I don't want my test results reported to the health department"                    
label(DT$fu1_q94_fu2___7) <- "I could not afford to miss work if I tested positive"                    
label(DT$fu1_q94_fu2___8) <- "I could not isolate myself if I tested positive"                    
label(DT$fu1_q94_fu2___9) <- "I did not have any COVID symptoms"                    
label(DT$fu1_q94_fu2___10) <- "Another reason (Specify)"
label(DT$fu1_q59_fu2___1) <- "I don't have to worry about getting COVID-19"                    
label(DT$fu1_q59_fu2___2) <- "I don't have COVID-19 now"                    
label(DT$fu1_q59_fu2___3) <- "I can be around others without giving the virus to them"                    
label(DT$fu1_q59_fu2___4) <- "I can be around others without getting the virus from them"                    
label(DT$fu1_q59_fu2___99) <- "Prefer not to answer"
label(DT$fu1_q60_fu2___1) <- "I will need to be admitted to the hospital"                    
label(DT$fu1_q60_fu2___2) <- "I will need to isolate myself from others"                    
label(DT$fu1_q60_fu2___3) <- "I will need to take off work"                    
label(DT$fu1_q60_fu2___99) <- "Prefer not to answer"                   
label(DT$fu1_q69_fu2___1) <- "Need to take time off work to get tested"                    
label(DT$fu1_q69_fu2___2) <- "Out of pocket costs for test"                    
label(DT$fu1_q69_fu2___3) <- "Out of pocket costs for transportation, childcare, or time off work to get tested"                    
label(DT$fu1_q69_fu2___4) <- "I do not know where to go to be tested"                    
label(DT$fu1_q69_fu2___5) <- "Pain or discomfort from the test"                    
label(DT$fu1_q69_fu2___6) <- "Saliva"                    
label(DT$fu1_q69_fu2___7) <- "Concern about others handling my personal data"                    
label(DT$fu1_q69_fu2___8) <- "Other"                    
label(DT$fu1_q69_fu2___9) <- "None of the above"                    
label(DT$fu1_q69_fu2___98) <- "Don't know"                   
label(DT$fu1_q69_fu2___99) <- "Prefer not to answer"
label(DT$fu1_q71_fu2___1) <- "There are other people in my household"                    
label(DT$fu1_q71_fu2___2) <- "There are children in my household"                    
label(DT$fu1_q71_fu2___3) <- "There are older adults in my household"                    
label(DT$fu1_q71_fu2___4) <- "I don't have a good place where I could isolate"                    
label(DT$fu1_q71_fu2___5) <- "No one to help me if I am sick"                    
label(DT$fu1_q71_fu2___6) <- "Lost income or wages"                    
label(DT$fu1_q71_fu2___7) <- "People might blame me or treat me badly"                    
label(DT$fu1_q71_fu2___8) <- "Other"                    
label(DT$fu1_q71_fu2___9) <- "None of the above"                    
label(DT$fu1_q71_fu2___98) <- "Don't know"                   
label(DT$fu1_q71_fu2___99) <- "Prefer not to answer"                   
label(DT$fu1_q40_fu2___1) <- "I want to keep my family safe"                    
label(DT$fu1_q40_fu2___2) <- "I want to keep my community safe I want to keep myself safe"                    
label(DT$fu1_q40_fu2___3) <- "I have a chronic health problem, like asthma or diabetes"                    
label(DT$fu1_q40_fu2___4) <- "My doctor told me to get a COVID-19 vaccine"                    
label(DT$fu1_q40_fu2___5) <- "I don't want to get really sick from COVID-19"                    
label(DT$fu1_q40_fu2___6) <- "I want to feel safe around other people"                    
label(DT$fu1_q40_fu2___7) <- "I believe life won't go back to normal until most people get a COVID-19 vaccine"        
label(DT$fu1_q40_fu2___8) <- "Required by my school or workplace"                    
label(DT$fu1_q40_fu2___9) <- "Required for travel"                    
label(DT$fu1_q40_fu2___10) <- "Other"                   
label(DT$fu1_q40_fu2___11) <- "Not Applicable"                   
label(DT$fu1_q40_fu2___12) <- "Prefer not to answer"
label(DT$fu1_q41_fu2___1) <- "I'm allergic to vaccines"                    
label(DT$fu1_q41_fu2___2) <- "I don't like needles"                    
label(DT$fu1_q41_fu2___3) <- "I'm not concerned about getting really sick from COVID-19"                    
label(DT$fu1_q41_fu2___4) <- "I'm concerned about side effects from the vaccine"                    
label(DT$fu1_q41_fu2___5) <- "I don't think vaccines work very well"                    
label(DT$fu1_q41_fu2___6) <- "I don't trust that the vaccine will be safe"                    
label(DT$fu1_q41_fu2___7) <- "I don't believe the COVID- 19 pandemic is as bad as some people say it is"                    
label(DT$fu1_q41_fu2___8) <- "I don't want to pay for it"                    
label(DT$fu1_q41_fu2___9) <- "I don't know enough about how well a COVID-19 vaccine works"                    
label(DT$fu1_q41_fu2___10) <- "Other"                   
label(DT$fu1_q41_fu2___11) <- "Not Applicable"                   
label(DT$fu1_q41_fu2___12) <- "Prefer not to answer"
label(DT$rapid_test_serial) <- ""
label(DT$mi_1) <- ""                           
label(DT$mi_2) <- ""                           
label(DT$mi_3) <- ""                           
label(DT$tmqq_1) <- ""                         
...
label(DT$nav_1) <- ""                          
label(DT$nav_2) <- ""                          
label(DT$self_test_1) <- ""                    
label(DT$self_test_2) <- ""                    
label(DT$fu_soc_harms_q1) <- ""                
label(DT$fu_soc_harms_explanation) <- ""       
label(DT$compensation_form_complete) <- ""     
label(DT$bl_date) <- "Baseline interview date"                        
label(DT$Pounds) <- "Weight in pounds"                         
label(DT$MI_Component) <- "MI component assigned"                   
label(DT$Peer_Component) <- "Peer component assigned"                 
label(DT$TMQQ_Component) <- "TMQQ component assigned"                 
label(DT$Access_Component) <- "Access component assigned"               
label(DT$Days_Since_BL) <- "Days since baseline interview"                  
label(DT$Racecheck) <- ""                      
label(DT$Raceunknown) <- ""                    
label(DT$Multirace) <- ""                      
label(DT$consent_ident) <- ""                  
label(DT$consent_zip_2) <- ""                  
```

```{r, eval=FALSE}
names(DT)[unlist(lapply(DT, function(x){is.null(attributes(x)$label)}))]
```
...
# R
saveRDS(DT[redcap_event_name == "screener_consent_arm_1", .SD, .SDcols = scon_vars],
                 file = "c:/Dbox/NCAP Data/Exported for NCAP Team/R/NCAP-Screener-Consent.rds")

saveRDS(DT[redcap_event_name == "screener_arm_1", .SD, .SDcols = scrn_vars],
                 file = "c:/Dbox/NCAP Data/Exported for NCAP Team/R/NCAP-Screening.rds")

saveRDS(DT[redcap_event_name == "baseline_interview_arm_2", .SD, .SDcols = bl_vars],
                 file = "c:/Dbox/NCAP Data/Exported for NCAP Team/R/NCAP-Baseline.rds")

saveRDS(DT[redcap_event_name == "followup_interview_arm_2", .SD, .SDcols = fu1_vars],
                 file = "c:/Dbox/NCAP Data/Exported for NCAP Team/R/NCAP-Follow1.rds")

saveRDS(DT[redcap_event_name == "followup_interview_arm_2b", .SD, .SDcols = fu2_vars],
                 file = "c:/Dbox/NCAP Data/Exported for NCAP Team/R/NCAP-Follow2.rds")
```
