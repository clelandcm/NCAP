...
meta_DT <- read.xlsx("NCAPTrial_DataDictionary_2022-08-17.xlsx", cols = 1:6)
...
### Due
```{r}
DT[redcap_event_name == "baseline_interview_arm_2", FU1_Due := ifelse(difftime(Sys.Date(), bl_date, units = "days") < 42, "FU1 Not Due Yet", "FU1 Due, 42+ Days Since Baseline")]
DT[, FU1_Due := factor(FU1_Due, levels = c("FU1 Not Due Yet", "FU1 Due, 42+ Days Since Baseline"))]
label(DT$FU1_Due) <- "First Follow-Up Due"

table1(~ FU1_Due, data = DT[redcap_event_name == "baseline_interview_arm_2",])
```
...
### Due
```{r}
DT[redcap_event_name == "baseline_interview_arm_2", FU2_Due := ifelse(difftime(Sys.Date(), bl_date, units = "days") < 84, "FU2 Not Due Yet", "FU2 Due, 84+ Days Since Baseline")]
DT[, FU2_Due := factor(FU2_Due, levels = c("FU2 Not Due Yet", "FU2 Due, 84+ Days Since Baseline"))]
label(DT$FU2_Due) <- "Second Follow-Up Due"

table1(~ FU2_Due, data = DT[redcap_event_name == "baseline_interview_arm_2",])
```
...
demog <- merge(DT[redcap_event_name == "screener_arm_1", .(participant_id, Hispanic, PHS_cat)],
...
```{r}
with(demog, 
     table(PHS_cat, Ethnicity = Hispanic, `Assigned Sex at Birth` = sex_Birth)) %>% 
  as.data.frame() %>%
  subset(Freq > 0) %>%
