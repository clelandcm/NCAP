...
table1(~ sex_Birth + edu_years_of_school +    
         ...
         discrimination_ethnicity + family_income, 
...
table1(~ Parent + household_famgen_3, 
       data = DT[redcap_event_name == "baseline_interview_arm_2",])
```
...
```{r}
PC <- REDCapR::redcap_read_oneshot(redcap_uri=uri, raw_or_label = "raw", token=tkns[2,2])$data
PC_n <- REDCapR::redcap_read_oneshot(redcap_uri=uri, raw_or_label = "raw", token=tkns[2,2])$data
...
table1(~ Parent + household_famgen_3, data = DT[redcap_event_name == "baseline_interview_arm_2",])
```

# COVID-19 Knowledge Questions (T/F)
```{r}
covknow <- melt(DT[redcap_event_name == "baseline_interview_arm_2", 
                   .SD, .SDcols = names(DT) %like% "participant_id|n_knowledge_\\d+$"], 
                id.vars = "participant_id", variable.name = "Item",
...
## Correct responses by item
**Item 1**: COVID-19 is a disease caused by infection with a virus. (True)     
**Item 2**: The COVID-19 virus CAN spread when a person is exposed to very small droplets that stay in the air - called airborne transmission. (True)     
**Item 3**: The COVID-19 virus CAN spread from someone who is infected but has no symptoms. (True)     
**Item 4**: Wearing a well-fitted mask DOES NOT HELP protect you and others by reducing the chance of spreading COVID-19. (False)     
**Item 5**: Black, Latino, and Indigenous people are at higher risk for severe COVID illness because of systemic health and social inequalities. (True)      
**Item 6**: Testing for COVID-19 is NOT an important part of controlling the spread of COVID. (False)      
**Item 7**: Weekly testing for COVID-19 is recommended for people who are not fully vaccinated. (True)     
**Item 8**: COVID-19 can NOT cause serious illness in people such as older adults or people with existing medical conditions. (False)      
**Item 9**: The ONLY WAY to test for COVID-19 is with a health care professional. (False)     
**Item 10**: There is no treatment for COVID-19. (False)    

```{r}
table1(~ Correct | Item_cat, data = covknow, overall=FALSE)
```
